...
export MARIADB_USER=${MARIADB_USER:-"nextcloud"} 
export MARIADB_PWD=${MARIADB_PWD:-"$(date +%s | sha256sum | base64 | head -c 32)"}
export MARIADB_DB=${MARIADB_DB:-"nextcloud"} 
export MARIADB_ROOTPWD=${MARIADB_ROOTPWD:-"$(date +%s | sha256sum | base64 | tail -c 32)"}
